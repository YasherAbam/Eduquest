...
import './inhouse-pending_details.css';

const InhousePendingDetailsModal = ({ isOpen, onClose, department, activityName, status, onVPAAApproval, onStatusChange }) => {
    ...
    return (
        <>
            <div className="inhouse_pending-details-modal-overlay">
                <div className="inhouse_pending-details-modal-content">
                    <div className="inhouse_pending-details-modal-header">
                        <h4>Pending In-House Activity Request Details</h4>
                        <button className="inhouse_pending-close-button" onClick={onClose}>×</button>
                    </div>
                    <div className="inhouse_pending-details-modal-body">
                        <div className="inhouse_pending-details-section">
                            <h5>Department Information</h5>
                            <div className="inhouse_pending-info-item">
                        ...
                        <div className="inhouse_pending-status-section">
                            <h5>Status Progress</h5>
                            <div className="inhouse_pending-steps-list">
                                <div className="inhouse_pending-step-item">
                                    <span className={`inhouse_pending-step-icon ${localSteps.requestLetter ? 'completed' : ''}`}>
                                        {localSteps.requestLetter ? '✓' : ''}
                                    </span>
                                    <span className="inhouse_pending-step-name">Request Letter</span>
                                </div>
                                <div 
                                    className={`inhouse_pending-step-item ${!localSteps.compliance ? 'clickable' : ''}`}
                                    onClick={() => handleStepClick('compliance')}
                                >
                                    <span className={`inhouse_pending-step-icon ${localSteps.compliance ? 'completed' : ''}`}>
                                        {localSteps.compliance ? '✓' : ''}
                                    </span>
                                    <span className="inhouse_pending-step-name">Compliance of Requirements</span>
                                </div>
                                <div 
                                    className={`inhouse_pending-step-item ${!localSteps.endorsedBySDS || localSteps.endorsedBySDS ? 'clickable' : ''}`}
                                    onClick={() => handleStepClick('endorsedBySDS')}
                                >
                                    <span className={`inhouse_pending-step-icon ${localSteps.endorsedBySDS ? 'completed' : ''}`}>
                                        {localSteps.endorsedBySDS ? '✓' : ''}
                                    </span>
                                    <span className="inhouse_pending-step-name">Endorsed by SDS Director</span>
                                </div>
                                <div 
                                    className={`inhouse_pending-step-item ${!localSteps.approvedByVPAA || localSteps.approvedByVPAA ? 'clickable' : ''}`}
                                    onClick={() => handleStepClick('approvedByVPAA')}
                                >
                                    <span className={`inhouse_pending-step-icon ${localSteps.approvedByVPAA ? 'completed' : ''}`}>
                                        {localSteps.approvedByVPAA ? '✓' : ''}
                                    </span>
                                    <span className="inhouse_pending-step-name">Approved by VPAA</span>
                                </div>
                                <div 
                                    className={`inhouse_pending-step-item ${!localSteps.narrativeReport || localSteps.narrativeReport ? 'clickable' : ''}`}
                                    onClick={() => handleStepClick('narrativeReport')}
                                >
                                    <span className={`inhouse_pending-step-icon ${localSteps.narrativeReport ? 'completed' : ''}`}>
                                        {localSteps.narrativeReport ? '✓' : ''}
                                    </span>
                                    <span className="inhouse_pending-step-name">Narrative Report</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {showVPAADialog && !localSteps.approvedByVPAA && (
                <div className="inhouse_pending-vpaa-dialog-overlay">
                    <div className="inhouse_pending-vpaa-dialog">
                        <h4>Have already received an approved letter from VPAA?</h4>
                        <div className="inhouse_pending-vpaa-dialog-buttons">
                            <button 
                                className="inhouse_pending-vpaa-button not-yet"
                                onClick={() => setShowVPAADialog(false)}
                            >
                                Not yet
                            </button>
                            <button 
                                className="inhouse_pending-vpaa-button yes"
...
export default InhousePendingDetailsModal;