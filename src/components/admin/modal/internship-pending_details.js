...
import './internship-pending_details.css';

const InternshipPendingDetailsModal = ({ isOpen, onClose, department, activityName, status, onVPAAApproval, onStatusChange }) => {
    ...
    return (
        <>
            <div className="internship_pending-details-modal-overlay">
                <div className="internship_pending-details-modal-content">
                    <div className="internship_pending-details-modal-header">
                        <h4>Pending Internship Activity Request Details</h4>
                        <button className="internship_pending-close-button" onClick={onClose}>×</button>
                    </div>
                    <div className="internship_pending-details-modal-body">
                        <div className="internship_pending-details-section">
                            <h5>Department Information</h5>
                            <div className="internship_pending-info-item">
                        ...
                        <div className="internship_pending-status-section">
                            <h5>Status Progress</h5>
                            <div className="internship_pending-steps-list">
                                <div className="internship_pending-step-item">
                                    <span className={`internship_pending-step-icon ${localSteps.requestLetter ? 'completed' : ''}`}>
                                        {localSteps.requestLetter ? '✓' : ''}
                                    </span>
                                    <span className="internship_pending-step-name">Request Letter</span>
                                </div>
                                <div 
                                    className={`internship_pending-step-item ${!localSteps.compliance ? 'clickable' : ''}`}
                                    onClick={() => handleStepClick('compliance')}
                                >
                                    <span className={`internship_pending-step-icon ${localSteps.compliance ? 'completed' : ''}`}>
                                        {localSteps.compliance ? '✓' : ''}
                                    </span>
                                    <span className="internship_pending-step-name">Compliance of Requirements</span>
                                </div>
                                <div 
                                    className={`internship_pending-step-item ${!localSteps.endorsedBySDS || localSteps.endorsedBySDS ? 'clickable' : ''}`}
                                    onClick={() => handleStepClick('endorsedBySDS')}
                                >
                                    <span className={`internship_pending-step-icon ${localSteps.endorsedBySDS ? 'completed' : ''}`}>
                                        {localSteps.endorsedBySDS ? '✓' : ''}
                                    </span>
                                    <span className="internship_pending-step-name">Endorsed by SDS Director</span>
                                </div>
                                <div 
                                    className={`internship_pending-step-item ${!localSteps.approvedByVPAA || localSteps.approvedByVPAA ? 'clickable' : ''}`}
                                    onClick={() => handleStepClick('approvedByVPAA')}
                                >
                                    <span className={`internship_pending-step-icon ${localSteps.approvedByVPAA ? 'completed' : ''}`}>
                                        {localSteps.approvedByVPAA ? '✓' : ''}
                                    </span>
                                    <span className="internship_pending-step-name">Approved by VPAA</span>
                                </div>
                                <div 
                                    className={`internship_pending-step-item ${!localSteps.narrativeReport || localSteps.narrativeReport ? 'clickable' : ''}`}
                                    onClick={() => handleStepClick('narrativeReport')}
                                >
                                    <span className={`internship_pending-step-icon ${localSteps.narrativeReport ? 'completed' : ''}`}>
                                        {localSteps.narrativeReport ? '✓' : ''}
                                    </span>
                                    <span className="internship_pending-step-name">Narrative Report</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {showVPAADialog && !localSteps.approvedByVPAA && (
                <div className="internship_pending-vpaa-dialog-overlay">
                    <div className="internship_pending-vpaa-dialog">
                        <h4>Have already received an approved letter from VPAA?</h4>
                        <div className="internship_pending-vpaa-dialog-buttons">
                            <button 
                                className="internship_pending-vpaa-button not-yet"
                                onClick={() => setShowVPAADialog(false)}
                            >
                                Not yet
                            </button>
                            <button 
                                className="internship_pending-vpaa-button yes"
...
export default InternshipPendingDetailsModal;